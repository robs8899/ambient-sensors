...
int main() {
  ...
  timer_init(); // DHT module uses Timer/Counter 1

  uart_puts("Sensor Node V. 1.00\r\n");

  dht_init(1, 3); // DHT pin at PC3
  ...
  do {
    ...
    dht_read(DHT22_TYPE, &rh, &temp, &fails);
    ...
    // format the results
    fmt(cb, fstr, node, stoa(t1, vbatt, 4, 3), stoa(t2, rh, 4, 1),
        stoa(t3, temp, 4, 1), dtoa(t4, fails, 2));
